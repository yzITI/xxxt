<script>
  import bannerImg from '$lib/images/banner.svg'
  import logoImg from '$lib/images/logo.svg'
  import { goto } from '$app/navigation'
  import swal from 'sweetalert2'

  function login () {
    window.location.href = 'https://auth.njsc.ltd/#/launch/xxxt'
  }
  ...
  function hit () {
    cot++
    if (cot >= 5) {
      swal.fire('Developer Mode!', 'You have entered the developer mode', 'success')
...
<section class="w-full h-screen relative bg-black text-white flex flex-col items-center justify-center">
  <img src={bannerImg} alt="banner" class="absolute w-full left-0 object-cover opacity-30" style="height: 120vh; top: -20vh;">
  <!-- svelte-ignore a11y-click-events-have-key-events -->
  <img on:click={hit} src={logoImg} alt="logo" class="absolute w-20 h-20 top-2 left-2">

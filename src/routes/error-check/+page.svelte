<script>
  ...
  async function analyze () {
    ...
    loading = true
    const msgs = [{ role: 'system', content: 'Explain the error message and provide short solutions' + (lang ? ' in Chinese' : '') }, { role: 'user', content: input }]
...
<div class="w-full min-h-screen p-4 sm:p-10 bg-gray-100">
  <h1 class="font-bold text-3xl flex items-center justify-between">
    <span>{lang ? '错误解释' : 'Error Explanation'}</span>
    <button on:click={() => { lang = !lang }}>
      <AIcon path={mdiTranslate} size="2rem" color="#333333" />
    </button>
  </h1>
  <p class="text-gray-500 my-2">{lang ? '请将错误信息到下面的输入框中' : 'Please paste your error message in the box below'}</p>
